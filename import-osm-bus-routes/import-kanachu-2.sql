...
from (
         SELECT t.name, t.destination, t.origin, brp.title, bpo.*, sp.*, bp.*
         FROM osm_transit.route t
                  ...
                  inner join odpt.busroute_pattern brp on brp.operator = 'Kanachu'
                      and brp.title ILIKE '%' || t.origin || '発%' || t.destination || '行'
         ...
         WHERE (t.operator LIKE '%神奈川中%' or network LIKE '%神奈川中%') and location is null
