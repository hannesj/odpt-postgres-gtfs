...
from (
         ...
         FROM osm_transit.route t
                  ...
                  inner join odpt.busroute_pattern brp on brp.operator = 'OdakyuBus'
             and brp.note ILIKE t.code || ':%' || t.origin || '～%' || t.destination || ':%'
         ...
         WHERE (t.operator LIKE '%小田急%' or network LIKE '%小田急%') and location is null
